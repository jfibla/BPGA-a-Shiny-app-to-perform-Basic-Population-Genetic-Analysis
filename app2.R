...
################################################################################  
  ...
  observeEvent(input$process_file, {
    ...
    # go to folder with the example file
      if (input$files == "exp1"){
    file_path <- "example/example_single.zip"
    file_ext <- tools::file_ext(file_path)
    base_name <- "example"
    } else {
    file_path <- "example/example_multi.zip"
    file_ext <- tools::file_ext(file_path)
    base_name <- "example"
      }
...
###############################################################################  
  ...
  observeEvent(input$run_admix, {
    ...
    for (i in 1:input$sliderK) {
      system(paste("www/admixture --cv", paste0("-j",Np), "PCA/input_admx.bed", i, "| tee log", i, ".out"))
